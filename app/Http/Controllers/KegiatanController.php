...
class KegiatanController extends Controller
{
    public function index(Request $request)
    {
        ...
        //get narasumber all
        $narasumber = \App\Models\Narasumber::all();
        
        if($request->ajax()){
            return datatables()->of($list_kegiatan)
                        ->addColumn('waktu', function($waktu)
                        {
                        $date = date("D, d F Y H:i", strtotime($waktu->waktu));
                        return $date;
                        })
                        ->addColumn('action', function($data){
                            $button = '<button type="button" name="detail" data-toggle="tooltip" name="detail" data-id="'.$data->id_kegiatan.'" class="detail btn btn-info btn-xs detail-kegiatan"><i class="fa fa-eye"></i> Detail</button>';
                            $button .= '&nbsp;&nbsp;';
                            $button .= '<a href="javascript:void(0)" data-toggle="tooltip"  data-id="'.$data->id_kegiatan.'" data-original-title="Edit" class="edit btn btn-sm btn-warning btn-xs edit-post"><i class="far fa-edit"></i> Ubah</a>';
                            $button .= '&nbsp;&nbsp;';
                            $button .= '<button type="button" name="delete" id="'.$data->id_kegiatan.'" class="delete btn btn-danger btn-xs"><i class="far fa-trash-alt"></i> Hapus</button>';     
                            return $button;
                        })
                        ->rawColumns(['action'])
                        ->addIndexColumn()
    ...
    return view('kegiatan.index',['provinces'=> $provinces,'narasumber'=>$narasumber,'regencies'=>$regencies]);
    }
    ...
    return view('kegiatan.show',compact('provinces'));
    }
    ...
    { 
        ...
        $post   =   kegiatan::updateOrCreate(['id_kegiatan' => $id],
                    [
                        ...
                        'kota' => $request->kota,
                        'id_narasumber'=> $request->narasumber,
